...
const getPotentiallyQualifierBasedPropValue = (infoValue) => {
	const { pkg } = getConfig();
	...
	if (typeof infoValue === "string") {
		...
		value =
			qualifierName && infoValue[qualifierName]
				? infoValue[qualifierName]
				: infoValue.default;
	}
...
const PROPS = {
	...
	description: null,
	author: (infoAuthor) => {
		const author = normaliseAuthor(infoAuthor || getPkgAuthor());

		return author ? author.name : null;
	},
	bundleID: (infoBundleID) => {
		if (infoBundleID) {
			return infoBundleID;
		}

		const { info, pkg } = getConfig();
		const author = normaliseAuthor(info.author || getPkgAuthor());
		const domain = (
			author && author.email ? author.email.split("@")[1] : "example.com"
		).split(".");
	...
	imagePath: (infoImagePath) => {
		return getPotentiallyQualifierBasedPropValue(infoImagePath);
	},
	launchSoundPath: (infoLaunchSoundPath) => {
		return getPotentiallyQualifierBasedPropValue(infoLaunchSoundPath);
	},
	contentWarning: null,
